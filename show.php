...
<div class="container" style="width: 80rem">
    ...
    if (isset($_GET['msg']) && $_GET['msg'] == 'success') {
        echo "<div class='alert alert-success' id='alert' role='alert'>
                Pizza has been edited
		   </div><script>window.history.pushState({}, document.title, '/pdo_pizza/show.php');</script>";
    ...
    <div class="row">
        <div class="col">
            ...
            $db->query('SELECT count(*) as count FROM pizza');
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    <th scope="col" class="text-center">Bodemformaat</th>
                    <th scope="col" class="text-center">Saus</th>
                    <th scope="col" class="text-center">Pizzatoppings</th>
                    <th scope="col" class="text-center">Pizzakruiden</th>
                ...
                <?php
                $db = new DatabaseController();
                $db->query('SELECT * FROM pizza');
                foreach ($db->resultset() as $item) {
					$kruiden = json_decode($item['kruiden']);
					$kruiden = implode(", ", $kruiden);

                    echo "<tr>
					<th scope='row' class='text-center'>$item[id]</th>
					<td class='text-center'>$item[bodemFormaat]</td>
					<td class='text-center'>$item[saus]</td>
					<td class='text-center '>$item[toppings]</td>
					<td class='text-center '>$kruiden</td>
